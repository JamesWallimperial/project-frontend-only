...
interface Props {
  devices: Device[];
  /** 0 = Back, 1..N = devices[0..N-1] */
  selectedIndex: number;
  onBack: () => void;
}

export default function LocalDevices({ devices, selectedIndex, onBack }: Props) {
  ...
  return (
    <div className={styles.app} role="region" aria-label="Local-only devices">
      <div className={styles.circleFrame}>
        ...
        <div className={styles.centerStack}>
          <div>
            <div className={styles.title}>Local-only</div>
          ...
          <ul
            ...
            aria-activedescendant={
              selectedIndex === 0 ? "local-back"
              : has ? `local-option-${listSel}` : undefined
            }
          >
            {devices.map((d, idx) => {
            ...
            {!has && <li className={styles.deviceItemEmpty}>No local-only devices</li>}
          </ul>
        </div>
      </div>
    </div>
