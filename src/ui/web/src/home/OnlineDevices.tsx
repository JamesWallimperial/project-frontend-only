...
interface Props {
  devices: Device[];
  /** Selection index: 0 = Back button, 1..N = devices[0..N-1] */
  selectedIndex: number;
  onBack: () => void;
}

export default function OnlineDevices({ devices, selectedIndex, onBack }: Props) {
  ...
  // For devices list, selection maps to (selectedIndex - 1)
  const listSel = Math.max(0, selectedIndex - 1);

  return (
    <div className={styles.app} role="region" aria-label="Online devices">
      <div className={styles.circleFrame}>
        ...
        <div className={styles.centerStack}>
          <div>
            <div className={styles.title}>Online devices</div>
          ...
          >
            ...
            {!has && <li className={styles.deviceItemEmpty}>No devices online</li>}
          </ul>
        </div>
      </div>
    </div>
